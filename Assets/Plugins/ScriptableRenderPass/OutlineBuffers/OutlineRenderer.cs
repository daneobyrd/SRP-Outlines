...
namespace Plugins.ScriptableRenderPass
{
    ...
    public class OutlineRenderer : ScriptableRendererFeature
    {
        ...
        {
            get => (opaqueSettings.enabled, transparentSettings.enabled);
            set => (opaqueSettings.enabled, transparentSettings.enabled) = value;
        }

        public override void Create()
        {
            if (EnabledPasses.Item1) { _lineworkOpaquePass      = new DrawToRT(opaqueSettings); }
                ...
                case EdgeDetectionMethod.Laplacian:
                    if (blur.enabled)
                    {
                        ...
                        // _blurPass.Init(opaqueSettings.customColorTargets[0].textureName,
                           _blurPass.Init(opaqueSettings.customColorTarget.textureName,
                    ...
                    else
                    {
                        ...
                        outlineSource = opaqueSettings.customColorTarget.textureName;
                        outlineID     = opaqueSettings.customColorTarget.GetNameID();
                    }
        ...
        {
            EnabledPasses = (opaqueSettings.enabled, transparentSettings.enabled);
