...
/// <summary>
/// Custom UI class for custom pass to Render Target
/// </summary>
[ShaderPassToRTDrawerAttribute(typeof(ShaderPassToRT))]
public class ShaderPassToRTDrawer
{
    class Styles
    {
        ...
        public static GUIContent callback =
            new GUIContent("Event", "Chose the Callback position for this render pass object.");
        ...
        public static GUIContent clearFlags = new GUIContent("Clear Flags",
                                                             "Clear Flags used when the render targets will are bound, before the pass renders.");
    }
    ...
    ShaderPassToRT m_CustomPassToRT;
    Type m_PassType => m_CustomPassToRT.GetType();

    void FetchProperties(SerializedProperty property)
    {
        m_Name              = property.FindPropertyRelative("profilerTag");
        m_Enabled           = property.FindPropertyRelative("enabled");
        m_TargetColorBuffer = property.FindPropertyRelative("customColorBuffer");
    ...
    void LoadUserProperties(SerializedProperty customPass)
    {
        ...
        var customPassFields =
            typeof(ShaderPassToRT).GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

        m_CustomPassUserProperties.Clear();
        foreach (var field in
                 m_PassType.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
        {
    ...
    internal void SetPass(ShaderPassToRT pass) => m_CustomPassToRT = pass;
    ...
    void DoCommonSettingsGUI(ref Rect rect)
    {
        if ((commonPassUIFlags & PassUIFlag.Name) != 0)
        {
            ...
            if (EditorGUI.EndChangeCheck())
                m_CustomPassToRT.settings.profilerTag = m_Name.stringValue;
    ...
    {
        var enabledSize = EditorStyles.boldLabel.CalcSize(Styles.enabled) +
                          new Vector2(Styles.reorderableListHandleIndentWidth, 0);
        ...
        {
            m_PassFoldout.boolValue = EditorGUI.Foldout(headerRect, m_PassFoldout.boolValue,
                                                        $"{m_Name.stringValue} ({m_PassType.Name})", true,
