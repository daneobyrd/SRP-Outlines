...
namespace Resources.RenderPass.OutlineBuffers
{
    ...
    public class ShaderPassToRT : ScriptableRenderPass
    {
        ...
        public ShaderPassToRT(OutlineSettings settings, string profilerTag, ComputeShader blurComputeShader, RenderPassEvent renderPassEvent, int depthBufferBits)
        {
            _settings = settings;
            _profilerTag = profilerTag;
            _blurComputeShader = blurComputeShader;
        ...
        public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
        {
            ...
            if (!createColorTexture && !createDepthTexture) return;
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            cmd.SetComputeVectorParam(_blurComputeShader, "_Size", new Vector4(width, height, 0, 0));
            
            switch (blurType)
            {
                case BlurType.ColorPyramid:
                    var colorKernel = _blurComputeShader.FindKernel("KColorGaussian");
                    cmd.SetComputeTextureParam(_blurComputeShader, colorKernel, "_Source", colorIntId, 0, RenderTextureSubElement.Color);
                    cmd.SetComputeTextureParam(_blurComputeShader, colorKernel, "_Destination", _blurIntId, 0);
                    cmd.DispatchCompute(_blurComputeShader, colorKernel, Mathf.CeilToInt(width / 8f), Mathf.CeilToInt(height / 8f), 1);
                    // ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                    var downsampleKernel = _blurComputeShader.FindKernel("KColorDownsample");
                    cmd.SetComputeTextureParam(_blurComputeShader, downsampleKernel, "_Source", colorIntId, 0, RenderTextureSubElement.Color);
                    cmd.SetComputeTextureParam(_blurComputeShader, downsampleKernel, "_Destination", _blurIntId, 0);
                    cmd.DispatchCompute(_blurComputeShader, downsampleKernel, Mathf.CeilToInt(width / 8f), Mathf.CeilToInt(height / 8f), 1);
                    break;
                case BlurType.GaussianPyramid:
                    var gaussKernel = _blurComputeShader.FindKernel("GAUSSIAN_PYRAMID");
                    cmd.SetComputeFloatParam(_blurComputeShader, "_gaussian_sigma", edge.gaussianSigma);
                    cmd.SetComputeTextureParam(_blurComputeShader, gaussKernel, "_Source", colorIntId, 0, RenderTextureSubElement.Color);
                    cmd.SetComputeTextureParam(_blurComputeShader, gaussKernel, "_Destination", _blurIntId, 0);
                    cmd.DispatchCompute(_blurComputeShader, gaussKernel, Mathf.CeilToInt(width / 8f), Mathf.CeilToInt(height / 8f), 1);
                    break;
                default:
                    blurType = BlurType.GaussianPyramid;
                    break;
